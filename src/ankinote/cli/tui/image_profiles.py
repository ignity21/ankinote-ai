"""Interactive image-provider-profile submenu."""
...
_VENDOR_OPTIONS = (*IMAGE_PROVIDERS.keys(), CUSTOM_VENDOR)


def _model_options(vendor: str) -> list[str]:
    return get_image_provider_models(vendor) if vendor != CUSTOM_VENDOR else []
...
def run_image_profiles_menu(settings: Settings) -> None:
    """Loop the image-profile submenu until the user picks "Back"."""
    while True:
        choices = [
            questionary.Choice(_label(name, settings.active_image_provider), value=name)
        ...
        selection = questionary.select("Image provider profiles", choices=choices).ask()
...
def _add(settings: Settings) -> None:
    ...
    default_name = unique_name(
        "Custom" if vendor == CUSTOM_VENDOR else vendor, set(settings.image_providers)
    )
    name = questionary.text("Profile name", default=default_name).ask()
    if not name:
        return
    base_url = questionary.text(
        "Base URL", default=IMAGE_PROVIDERS.get(vendor, {}).get("api_base", "")
    ).ask()
    if base_url is None:
        return
    try:
        ...
        return
    settings.active_image_provider = name
...
def _rename(settings: Settings, name: str, profile) -> bool:
    ...
    if not new_name:
        return False
    try:
        settings.active_image_provider = rename_profile(
            settings.image_providers, settings.active_image_provider, name, new_name
        )
    except ValueError as exc:
        questionary.print(str(exc), style="fg:red")
        return False
...
def _remove(settings: Settings, name: str, profile) -> bool:
    if not questionary.confirm(f"Remove profile {name!r}?", default=False).ask():
        return False
    try:
        settings.active_image_provider = remove_profile(
            settings.image_providers, settings.active_image_provider, name
        )
    except ValueError as exc:
        questionary.print(str(exc), style="fg:red")
        return False
...
def _edit(settings: Settings, name: str) -> None:
    profile = settings.image_providers[name]
